...
export class MyMCP extends McpAgent {
	...
	async init() {
		...
		this.server.tool(
			...
			{
				chain: z.enum(["Base", "Ethereum", "Avalanche"])
			...
			async ({ chain }) => {
				...
				if (!apiData) {
					return {
						content: [{ type: "text", text: "Failed to retrieve loan pool data" }],
					};
				}

				const filteredPools = apiData.data.filter((pool: any) =>
					pool.symbol === 'USDC' && pool.chain === chain
				);
				...
				const formattedPools: LoanPool[] = filteredPools.map((pool: any) => {
					...
					if (pool.tvlUsd > 100000000) {
						riskLevel = 'Low Risk';
					} else if (pool.tvlUsd > 10000000 && pool.apy >= 5 && pool.apy <= 30) {
				...
				const formattedPoolsText = topPools.map(pool => this.formatLoanPool(pool));
		...
		this.server.tool(
			...
			"Get high yield USDC loan pools",
			{
				...
				maxRisk: z.enum(["Low Risk", "Medium Risk", "High Risk"])
			...
			async ({ minApy, maxRisk }) => {
				...
				if (filteredPools.length === 0) {
					return {
						content: [
							{ type: "text", text: `No USDC loan pools found with APY >= ${minApy}% and risk level <= ${maxRisk}` },
						],
					};
				}
